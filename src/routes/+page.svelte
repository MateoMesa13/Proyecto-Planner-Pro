<script lang="ts">
    ...
    async function handleLogin(event: Event) {
        ...
        try {
            ...
            if ((err as any)?.message && (err as any).message.includes('Email not confirmed')) {
                error = 'Correo no verificado, revisa tu bandeja de email';
            } else {
                ...
                <form class="space-y-4 md:space-y-6" on:submit={handleLogin}>
                    ...
                    {#if registered}
                        <div class="p-4 mb-4 text-sm text-green-800 rounded-lg bg-green-50 dark:bg-gray-800 dark:text-green-400" role="alert">
                            <span class="font-medium">¡Registro exitoso!</span> Por favor inicia sesión con tus credenciales.
                        </div>
                    {/if}
                    <div>
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <a href="/" class="text-sm font-medium text-white hover:underline dark:text-primary-500">Olvidé mi contraseña</a>
                    </div>
                    <button 
                        type="submit" 
